...
class SettingWidget extends ConsumerWidget {
  ...
  Future<void> _pickTemplateFile(BuildContext context, WidgetRef ref, String prefKey) async {
    final settingsNotifier = ref.read(settingsProvider.notifier);
    final vaultPath = ref.read(settingsProvider).vaultPath;

    if (vaultPath == null || vaultPath.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('請先設定Vault路徑!')),
      );
      return;
    }
    final selectedAbsolutePath = await Navigator.of(context).push<String>(
      MaterialPageRoute(
        builder: (_) => DocumentWidget(isSelectMode: true),
      ),
    );

    if (selectedAbsolutePath != null) {
      final selectedRelativePath = p.relative(selectedAbsolutePath, from: vaultPath);
        ...
        child: ListView(
          children: [
            ...
            TemplateSettingWidget(
              ...
              buttonPressedEvent: () => _pickTemplateFile(context, ref, SettingsNotifier.itineraryTemplatePathKey),
            ),
            TemplateSettingWidget(
              templateType: '行程單日模板',
              templatePath: settingsState.itineraryDayTemplatePath,
              buttonPressedEvent: () => _pickTemplateFile(context, ref, SettingsNotifier.itineraryDayTemplatePathKey),
            ),
            TemplateSettingWidget(
              templateType: '景點清單模板',
              templatePath: settingsState.locationListTemplatePath,
              buttonPressedEvent: () => _pickTemplateFile(context, ref, SettingsNotifier.locationListTemplatePathKey),
            ),
            TemplateSettingWidget(
              templateType: '景點項目模板',
              templatePath: settingsState.locationTemplatePath,
              buttonPressedEvent: () => _pickTemplateFile(context, ref, SettingsNotifier.locationTemplatePathKey),
            ),
          ],
        ),
      ),
